...
fn main() {
    ...
    while i <= threads { // repeat this code to generate each thread
        ...
        let handle = thread::spawn(move || { // create a thread and save its handle which will be used later
            ...
            while i_loc <= cycles_per_thread { // do for each π needed to be generated by this thread
                let area_mc = generate_area_mc(points_tot, min, max, a, b); // calculate an area value with MC method
                let area_traps = calc_area_trap(trap, a, b); // calculate an area value with trapezoids method
    ...
    for received in rx { // for each value that the rx Receiver<f64> gets from the tx Sender<f64> object, store it to the received constant (an f64)
        ...
        if mc_res.len() as i32 == n { // once the area count is equal to the desired number of areass to be generated...
            ...
            println!("Mean: {area}, SE: {se}, STDEV: {stdev}, a: {a}, b: {b}, n: {length}, points: {points_tot}, t: {t} s"); // print result to the console
            ...
            writeln!(file, "Mean: {area}, SE: {se}, STDEV: {stdev}, a: {a}, b: {b}, n: {length}, points: {points_tot}, t: {t} s") // write results to the file
    ...
    for received in rx_trap { // for each value that the rx Receiver<f64> gets from the tx Sender<f64> object, store it to the received constant (an f64)
        ...
        if trap_res.len() as i32 == n { // once the area count is equal to the desired number of areass to be generated...
            ...
            println!("Mean: {area}, SE: {se}, STDEV: {stdev}, a: {a}, b: {b}, n: {length}, traps: {trap}, t: {t} s"); // print result to the console
            ...
            let mut file = OpenOptions::new()
                ...
                .open(file_name)
                .expect("Error reading file!");
            writeln!(file, "Mean: {area}, SE: {se}, STDEV: {stdev}, a: {a}, b: {b}, n: {length}, traps: {trap}, t: {t} s") // write results to the file
                .expect("Error writing to file"); // throw an error if there was a problem writing to the file 
            break; // break from the loop, so that the main thread can start the processing the results from the areas and volumes
        }
    }
...
fn f(x: f64) -> f64 {
    ...
    //return e^(-x^2.0);
    return e.powf(-1.0 * x.powf(2.0));
}

fn generate_area_mc(points_tot: i32, min: f64, max:f64, a: f64, b: f64) -> f64 {
    ...
    let mut i = 1; // index to count the number of generated points
        ...
        while i <= points_tot { // repeat until all points have been generated
            ...
            if y <= f(x) { // test if the point is within the circle
...
fn calc_area_trap(traps: i32, a: f64, b: f64) -> f64 {
    let range = b - a;
    ...
    while i <= traps {
        area += delta_x * (f(delta_x * f64::from(i - 1)) + f(delta_x * f64::from(i))) / 2.0;
        i += 1;
    }
